class TrajectoryManager{
  private:
    ...
    geometry_msgs::Quaternion real_orient;

    ros::Publisher pose_pub;
    ros::Publisher vel_lim_pub;
    ros::Publisher real_pos_pub;
    ros::Publisher real_orient_pub;
  ...
  public:
    TrajectoryManager(): generator(rd()), distribution(std::normal_distribution<>(0.0, 0.1)){
      // Subscribtions
      pose_pub = node.advertise<geometry_msgs::PoseStamped>("/command/pose", 50);
      vel_lim_pub = node.advertise<geometry_msgs::Twist>("/command/twist_limit", 50);
      real_pos_pub = node.advertise<geometry_msgs::Point>("/raw_pos", 50);
      real_orient_pub = node.advertise<geometry_msgs::Quaternion>("/raw_orient", 50);

      gnd_truth_pos = node.subscribe("/ground_truth/state", 50, &TrajectoryManager::pos_faker, this);
      enable_movement = node.subscribe("/moveit", 50, &TrajectoryManager::moveCallback, this);
    ...
    void pos_faker(const nav_msgs::Odometry& gnd_truth){
      ...
      real_orient = gnd_truth.pose.pose.orientation;
      //std::cout << real_orient << std::endl;

      real_pos_pub.publish(real_pos);
      real_orient_pub.publish(real_orient);
