class NavFilter{
  private:
    ...
    Eigen::Matrix<double,MSRMTS,1> z_k;         // Sensor measurement [quat; pos]
  ...
  public:
    ...
    NavFilter(){
      ...
      B_k.setZero();
      H_k.setZero();
      H_k.block(0,0,4,4).setIdentity(); // Quaternion model
      H_k.block(4,4,3,3).setIdentity(); // Position model
      H_k.block(7,7,3,3).setIdentity(); // Velocity model

      // Process noise covariance (tuning parameters)
      Q_k.setIdentity();
      Q_k.block(0,0,4,4).diagonal().setConstant(0.01);     // Quaternion related-covariance
      Q_k.block(4,4,3,3).diagonal().setConstant(0.1);     // Position-related covariance
      Q_k.block(7,7,3,3).diagonal().setConstant(0.1);     // Velocity-related covariance
      ...
      R_k.block(0, 0, 4, 4).diagonal() << 0.02, 0.02, 0.02, 0.02;    // Quaternion measurement noise
      R_k.block(4, 4, 3, 3).diagonal() << 0.1, 0.1, 0.1;    // Position measurement noise
      R_k.block(7, 7, 3, 3).diagonal() << 0.1, 0.1, 0.1;    // Velocity measurement noise


      // Subscribtions
      sub_pos = node.subscribe("/raw_pos", 50, &NavFilter::posCallback, this);
      sub_orient = node.subscribe("/raw_orient", 50, &NavFilter::orientCallback, this);
      ...
      // Publications
      odom_pub = node.advertise<nav_msgs::Odometry>("/fused_state/odom", 50);
    ...
    void propagateFilter(){
      ...
      F_k.block(10,10,3,3) = Eigen::Matrix<double,3,3>::Identity();   // Gyro bias Propagation
      F_k.block(13,13,3,3) = Eigen::Matrix<double,3,3>::Identity();   // Accel Propagation

      F_k.block(4,13,3,3) = Eigen::Matrix<double,3,3>::Identity();  // Bias for acceleration component
    ...
    void posCorrection(){
      ...
      y_k.block(4,0,3,1) = z_k.block(4,0,3,1) - H_k.block(4,0,3,STATES) * x_k_p;
      S_k.block(4,4,3,3) = R_k.block(4,4,3,3) + H_k.block(4,0,3,STATES) * P_k_p * H_k.block(4,0,3,STATES).transpose();
      K_k.block(0,4,STATES,3) = P_k_p * H_k.block(4,0,3,STATES).transpose() * S_k.block(4,4,3,3).inverse();

      // Update
      x_k = x_k_p + K_k.block(0,4,STATES,3) * y_k.block(4,0,3,1);
      P_k = (Eigen::Matrix<double,STATES,STATES>::Identity() - K_k.block(0,4,STATES,3) * H_k.block(4,0,3,STATES)) * P_k;
    ...
    void orientCorrection(){
      // Correction for Quaternion measurement
      /*y_k.block(0,0,4,1) = z_k.block(0,0,4,1) - H_k.block(0,0,4,STATES) * x_k_p;
      S_k.block(0,0,4,4) = R_k.block(0,0,4,4) + H_k.block(0,0,4,STATES) * P_k_p * H_k.block(0,0,4,STATES).transpose();
      K_k.block(0,0,STATES,4) = P_k_p * H_k.block(0,0,4,STATES).transpose() * S_k.block(0,0,4,4).inverse();

      // Update
      x_k = x_k_p + K_k.block(0,0,STATES,4) * y_k.block(0,0,4,1);
      P_k = (Eigen::Matrix<double,STATES,STATES>::Identity() - K_k.block(0,0,STATES,4) * H_k.block(0,0,4,STATES)) * P_k;*/
      x_k.block(0,0,4,1) = z_k.block(0,0,4,1);
    ...
    void velCorrection(){
      // Correction for velocity measurement
      y_k.block(7,0,3,1) = z_k.block(7,0,3,1) - H_k.block(7,0,3,STATES) * x_k_p;
      S_k.block(7,7,3,3) = R_k.block(7,7,3,3) + H_k.block(7,0,3,STATES) * P_k_p * H_k.block(7,0,3,STATES).transpose();
      K_k.block(0,7,STATES,3) = P_k_p * H_k.block(7,0,3,STATES).transpose() * S_k.block(7,7,3,3).inverse();

      // Update
      x_k = x_k_p + K_k.block(0,7,STATES,3) * y_k.block(7,0,3,1);
      P_k = (Eigen::Matrix<double,STATES,STATES>::Identity() - K_k.block(0,7,STATES,3) * H_k.block(7,0,3,STATES)) * P_k;
    ...
    void orientCallback(const geometry_msgs::Quaternion& orient_meas){
      z_k(0) = orient_meas.x;
      z_k(1) = orient_meas.y;
      z_k(2) = orient_meas.z;
      z_k(3) = orient_meas.w;
    ...
    void velCallback(const geometry_msgs::TwistWithCovariance& vel_meas){
      ...
      z_k(7) = vel_meas.twist.linear.x;
      z_k(8) = vel_meas.twist.linear.y;
      z_k(9) = vel_meas.twist.linear.z;
    ...
    void publishState(){
      ...
      tf::Quaternion quat_tf = tf::Quaternion(x_k(0), x_k(1), x_k(2), x_k(3)).normalize();
    ...
    void updateOmega(){
      //ROS_INFO("HERE Om");
      /*Omega_k <<             0,  w_k(2)+x_k(12), -w_k(1)+x_k(11), w_k(0)+x_k(10),
               -w_k(2)+x_k(12),               0,  w_k(0)+x_k(10), w_k(1)+x_k(11),
                w_k(1)+x_k(11), -w_k(0)+x_k(10),               0, w_k(2)+x_k(12),
               -w_k(0)+x_k(10), -w_k(1)+x_k(11), -w_k(2)+x_k(12),              0;
      return;*/
